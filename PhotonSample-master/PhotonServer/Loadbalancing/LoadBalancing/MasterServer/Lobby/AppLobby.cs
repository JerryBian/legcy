...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    ...
    public class AppLobby
    {
        ...
        public AppLobby(GameApplication application, string lobbyName, AppLobbyType lobbyType)
            : this(application, lobbyName, lobbyType, 10, TimeSpan.FromSeconds(15))
        {
        }
        ...
        public HashSet<PeerBase> Peers => peers;
        ...
        protected virtual OperationResponse HandleCreateGame(MasterClientPeer peer, OperationRequest operationRequest)
        {
            ...
            GameState gameState;
            bool gameCreated;
            if (!this.TryCreateGame(operation, operation.GameId, operation.CreateIfNotExists, operation.GameProperties, out gameCreated, out gameState, out response))
            {
                return response;
            }
